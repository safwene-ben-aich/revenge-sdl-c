...
void main_stage_2(SDL_Surface *ecran, int stage, int *stage_dispo, int *difficulter)
{
  Mix_AllocateChannels(10);
    ...
    int cpt_s_j=0 , cpt_s_w =0 ;
FILE *fichier=NULL;
SDL_Rect position_base , camera ,position_objet,position_texte;
position_texte.x=80;
position_texte.y=50;
int compteur=0,cpt_sound=300,surf=0,dans_l_air=0  ,ending=0 , fix_map=0,cpt_map=0 ,cpt_u_r=0 ,mvt_up_right=0 ;
char temps[20]="";
Uint8 g=0,r=255;
...
int arrested=0,jouer=0,cpt_jeu=0,cpt_aide=0, continuer=1,i=0,cpt=0,mvt_up=0,i_pause=1;
...
surf =0 ;
     break ;
 case SDLK_ESCAPE :
   Pause(ecran,camera,position_objet,2) ;
    break ;
...
if ( (ending==1) && (arrested !=1))
    {
  if (*(stage_dispo)<3)     {*stage_dispo =3 ;}
...
loun=SDL_LireCouleur2(test_map,(position_objet.x+camera.x+(noir->w)-20),(position_objet.y+(noir->h)+i-5));
...
{
SDL_LockSurface(test_map);
loun=SDL_LireCouleur2(test_map,(position_objet.x+camera.x+(noir->w)+10),(position_objet.y+(noir->h)-i));
SDL_UnlockSurface(test_map);
if (loun.r!=0 && loun.g!=0 && loun.b!=0) {test ++ ;}
}
test = max_pix-test  ;
...
{
SDL_LockSurface(test_map);
loun=SDL_LireCouleur2(test_map,(position_objet.x+camera.x+(noir->w/2)-30 + i ),(position_objet.y+(noir->h)));
SDL_UnlockSurface(test_map);
...
_Couleur_ loun;
 for (i=0;i<50;i++)
{SDL_LockSurface(test_map);
loun=SDL_LireCouleur2(test_map,(position_objet.x+noir->w+camera.x+10),position_objet.y+(noir->h)-i);
SDL_UnlockSurface(test_map);
if (loun.r!=0 && loun.g!=0 && loun.b!=0)
{ test ++ ;}}
